...
function setRunning(v){
  ...
  stopBtn.disabled = !v;
  recordBtn.textContent = v ? 'Start Automatic Voice Changer' : 'Start Automatic Voice Changer';
}
...
async function resemble(path, options = {}){
  ...
  if(!response.ok) {
    const detail = data.error || data.message || data.detail || `Resemble HTTP ${response.status}`;
...
async function applySink(save = true){
  selectedOutput = outputDevice.value || 'default';
  try{
...
async function playResembleAudio(base64, mime = 'audio/wav'){
  const blob = decodeBase64Audio(base64, mime);
...
async function loadVoices(){
  ...
  const data = await resemble('/voices');
  const previous = voiceUuid.value;
  voiceUuid.innerHTML = '';
  const voices = data.items || data.voices || [];
...
async function connect(){
  try{
    ...
    setRunning(true);
    setStatus('Resemble browser mode ready');
    transcriptEl.textContent = 'Ready. Start the automatic voice changer and just talk — no Stop button is needed between sentences.';
...
async function testAudio(){
  if(!running) return;
  try{
    const voice = voiceUuid.value.trim();
    if(!voice) throw new Error('Select a Resemble custom voice first.');
    setStatus('Resemble TTS: generating test voice…');
    meterFill.style.width = '55%';
    const data = await resemble('/../synthesize');
    // The API server and synthesis server are separate, so use the synthesis host directly below.
    const response = await fetch(`${RESEMBLE_SYNTH}/synthesize`, {
      ...
      body:JSON.stringify({voice_uuid:voice,data:'Hello! This is the Resemble browser voice test.',output_format:'wav',sample_rate:48000})
    });
    const result = await response.json().catch(()=>({}));
    if(!response.ok) throw new Error(result.error || result.message || `Resemble TTS HTTP ${response.status}`);
    await playResembleAudio(result.audio_content, 'audio/wav');
...
async function transcribeAndSpeak(wav){
  ...
  processingPromise = (async()=>{
    try{
      ...
      transcriptEl.textContent = 'Speech captured — Resemble is transcribing automatically…';
      meterFill.style.width = '55%';
      const form = new FormData();
      form.append('file', wav, 'voice.wav');
      const created = await resemble('/speech-to-text', {method:'POST', body:form});
      ...
      for(let attempt=0; attempt<30; attempt++){
        ...
        const result = await resemble(`/speech-to-text/${encodeURIComponent(uuid)}`);
      ...
      setStatus('Resemble TTS: repeating your speech…');
      const voice = voiceUuid.value.trim();
      const ttsText = prompt.value.trim() ? `<speak prompt="${prompt.value.trim().replace(/"/g,'&quot;')}">${text.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;')}</speak>` : text;
      const response = await fetch(`${RESEMBLE_SYNTH}/synthesize`, {
        method:'POST',
        headers:{Authorization:`Bearer ${requireKey()}`,'Content-Type':'application/json'},
        body:JSON.stringify({voice_uuid:voice,data:ttsText,output_format:'wav',sample_rate:48000})
      });
      const data = await response.json().catch(()=>({}));
      if(!response.ok) throw new Error(data.error || data.message || `Resemble TTS HTTP ${response.status}`);
      await playResembleAudio(data.audio_content, 'audio/wav');
      meterFill.style.width = '100%';
      setStatus('Listening again…');
    }catch(err){
      setStatus('Automatic voice error: ' + err.message);
      transcriptEl.textContent = err.message;
      meterFill.style.width = '0%';
    }finally{
...
async function startAutomaticVoiceChanger(){
  ...
  try{
    ...
    listening = true;
    recordBtn.textContent = 'Automatic Listening…';
    setStatus('Listening… talk normally');
    transcriptEl.textContent = 'Speak naturally. The browser detects when you stop talking and automatically sends the sentence to Resemble.';
    meterFill.style.width = '25%';

    processorNode.onaudioprocess = event => {
      if(!listening || speaking || processingPromise) return;
      const input = event.inputBuffer.getChannelData(0);
      const copy = new Float32Array(input.length);
      copy.set(input);
      const level = rms(input);
      const now = performance.now();
      const dt = Math.max(1, Math.min(100, now - lastProcessTime));
      lastProcessTime = now;
      ...
      if(level > threshold){
        ...
        meterFill.style.width = `${Math.min(70, 25 + level * 900)}%`;
      } else if(speechStarted){
        recordedChunks.push(copy);
        silenceMs += dt;
        speechMs += dt;
        if(silenceMs >= 750 && speechMs >= 350){
          ...
          transcribeAndSpeak(wav);
        }
      }
    };
  }catch(err){
    await stopAutomaticVoiceChanger();
...
async function stopAutomaticVoiceChanger(){
  ...
  speechMs = 0;
  recordedChunks = [];
  if(processorNode){ try{ processorNode.disconnect(); }catch{} }
  ...
  stream = null;
  audioContext = null;
  if(running) recordBtn.textContent = 'Start Automatic Voice Changer';
}
...
recordBtn.onclick = () => listening ? stopAutomaticVoiceChanger() : startAutomaticVoiceChanger();
stopBtn.onclick = stop;
chooseOutputBtn.onclick = chooseOutput;
refreshAudioBtn.onclick = async () => { try{ await loadAudioDevices(); setStatus('Browser audio outputs refreshed'); }catch(err){ setStatus('Audio output error: ' + err.message); } };
