...
function wavDecode(buf){
  ...
  while(p+8<=buf.length){
    ...
    if(id==="fmt "){const format=buf.readUInt16LE(p);channels=buf.readUInt16LE(p+2);rate=buf.readUInt32LE(p+4);bits=buf.readUInt16LE(p+14);if(format!==1||bits!==16)throw new Error("Only PCM 16-bit WAV is supported.");}
    if(id==="data"){data=buf.subarray(p,p+n);break;}
    p+=n+(n&1);
  }
  if(!data||!rate)throw new Error("Invalid WAV data.");
...
function spectrogram(x){
  ...
  for(let i=0;i<P;i++){pad[P-1-i]=x[Math.min(i,x.length-1)]??0;pad[P+x.length+i]=x[Math.max(0,x.length-1-i)]??0;}
  pad.set(x,P);
  ...
  for(let f=0;f<frames;f++){
    ...
    for(let k=0;k<=512;k++){
      let re=0,im=0;
      for(let n=0;n<N;n++){const a=2*Math.PI*k*n/N,v=pad[off+n]*w[n];re+=v*Math.cos(a);im-=v*Math.sin(a);}
...
function encodeWav(x,rate){
  ...
  for(let i=0;i<x.length;i++){const v=Math.max(-1,Math.min(1,x[i]));b.writeInt16LE(v<0?v*32768:v*32767,44+i*2);}
  return b;
}
...
const server=http.createServer(async(req,res)=>{
  ...
  if(req.url==="/health")return reply(res,200,{ok:true,models:!!(refModel&&convModel),voice:!!target});
  ...
  if(req.method==="GET"){
    ...
    if(file.startsWith(ROOT) && file!==path.join(ROOT,"server.mjs")){
      try{const data=await fs.readFile(file);res.writeHead(200,{"Content-Type":MIME[path.extname(file)]||"application/octet-stream","Cache-Control":"no-store"});return res.end(data);}catch{}
    }
  }
...
wss.on("connection",ws=>{
  ...
  ws.on("message",raw=>{
    busy=busy.then(async()=>{
      ...
      }catch(e){console.error("conversion:",e.message);if(ws.readyState===1)ws.send(JSON.stringify({type:"error",error:e.message}));}
    });
  });
});
