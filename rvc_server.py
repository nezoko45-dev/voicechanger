...
def ensure_dependencies():
    packages = ["rvc-python==0.1.5", "numpy>=1.26,<3", "soundfile>=0.12.1", "websockets>=12,<16"]
    try:
...
def wav_to_packet(path: Path) -> bytes:
    ...
    return int(rate).to_bytes(4, "little") + np.asarray(audio, dtype=np.int16).tobytes()


def convert_chunk(pcm: bytes, sample_rate: int) -> bytes:
    if converter is None: load_converter()
    with tempfile.TemporaryDirectory(prefix="rvc_chunk_") as d:
        src, dst = Path(d) / "input.wav", Path(d) / "output.wav"
...
async def send_models(ws):
    await ws.send(json.dumps({"type": "models", "models": [p.name for p in find_models()]}))


async def handler(ws):
    await ws.send(json.dumps({"type": "ready", "device": get_device(), "models": [p.name for p in find_models()]}))
    upload = None
    async for message in ws:
        if isinstance(message, str):
            if message == "LIST_MODELS":
                await send_models(ws); continue
            if message.startswith("MODEL "):
                try:
                    load_converter(message[6:].strip() or None)
                    await ws.send(json.dumps({"type": "model_loaded", "model": Path(current_model).name}))
                except Exception as exc:
                    await ws.send(json.dumps({"type": "error", "message": str(exc)}))
                continue
            if message.startswith("UPLOAD_MODEL "):
                try:
                    _, name, size_text = message.split(" ", 2)
                    name = Path(name).name; size = int(size_text)
                    if not name.lower().endswith(".pth"): raise ValueError("Only .pth RVC models are supported.")
                    if size <= 0 or size > MAX_MODEL_SIZE: raise ValueError("Model file is too large or empty.")
                    MODEL_DIR.mkdir(exist_ok=True)
                    path = MODEL_DIR / name
                    if upload:
                        upload["file"].close()
                    upload = {"path": path, "size": size, "received": 0, "file": path.open("wb")}
        ...
        if upload is not None:
            try:
                upload["file"].write(message); upload["received"] += len(message)
                if upload["received"] > upload["size"]: raise ValueError("Model upload exceeded declared size.")
                if upload["received"] == upload["size"]:
                    upload["file"].close(); path = upload["path"]; upload = None
        ...
        try:
            ...
            output = await asyncio.to_thread(convert_chunk, message[4:], rate)
...
async def main():
    ...
    else: print("No model yet. Upload one from the webpage.")
