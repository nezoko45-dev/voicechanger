...
def downsample_48_to_16(raw):
    # Audio Repeater/VAC normally runs at 48 kHz. Pick every third int16 sample.
    if len(raw) < 6:
        return b""
    ...
    while not stop_event.is_set():
        try:
            ...
            if not stop_event.is_set():
                with lock:
                    state["error"] = "TTS receive: " + str(exc)
            break
        if msg is None:
            break
...
def stt_receiver(ws):
    ...
    while not stop_event.is_set():
        try:
            ...
            if not stop_event.is_set():
                with lock:
                    state["error"] = "STT receive: " + str(exc)
            break
...
def start_audio():
    global input_stream, output_stream, stt_ws, tts_ws, threads
    stop_audio()
    stop_event.clear()
    with lock:
        key = state["api_key"].strip()
        inp = state["input_device"]
        out = state["output_device"]
        voice = state["voice"]
        state["error"] = ""
    if not key:
        raise RuntimeError("Deepgram API key is required")

    in_idx = device_index(inp, True)
    out_idx = device_index(out, False)
    if in_idx is None:
        raise RuntimeError("Audio Repeater output/input device was not found")
    if out_idx is None:
        raise RuntimeError("VAC/output device was not found")
    ...
    threads = [
        threading.Thread(target=stt_sender, args=(stt_ws,), daemon=True),
        threading.Thread(target=stt_receiver, args=(stt_ws,), daemon=True),
        threading.Thread(target=tts_receiver, args=(tts_ws, output_stream), daemon=True),
    ]
    for t in threads:
        t.start()
    with lock:
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            if path == "/config":
                with lock:
                    state["api_key"] = body.get("api_key", "")
                    state["input_device"] = body.get("input_device")
                    state["output_device"] = body.get("output_device")
                    state["voice"] = body.get("voice", "aura-2-thalia-en")
                self._send({"ok": True})
            elif path == "/start":
                start_audio()
                self._send({"ok": True})
