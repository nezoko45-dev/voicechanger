...
async function ensureOutputAudioContext() {
  ...
  if (!outputAudioContext || outputAudioContext.state === "closed") {
    try { outputAudioContext = wanted !== "default" ? new AudioContext({ sinkId: wanted }) : new AudioContext(); }
    catch (e) { log(`OUTPUT CONTEXT FALLBACK: ${e.message}`); outputAudioContext = new AudioContext(); }
  }
...
function stopScheduledAudio() { for (const node of scheduledAudio) { try { node.stop(); } catch {} } scheduledAudio = []; nextAudioTime = 0; }
function stopPlayback() { stopScheduledAudio(); }
async function playWav(blob) {
  stopPlayback(); const ctx = await ensureOutputAudioContext(); const buffer = await ctx.decodeAudioData(await blob.arrayBuffer());
  const source = ctx.createBufferSource(); source.buffer = buffer; source.connect(ctx.destination);
  const start = Math.max(ctx.currentTime + 0.01, nextAudioTime || 0); source.start(start); nextAudioTime = start + buffer.duration;
  scheduledAudio.push(source); source.onended = () => { scheduledAudio = scheduledAudio.filter((n) => n !== source); };
}
function enqueueAudioChunk(audio, generation, ctx) {
  if (!audio || generation !== conversionGeneration || !ctx) return;
  try {
    const samples = audio instanceof Float32Array ? audio : new Float32Array(audio); if (!samples.length) return;
    const buffer = ctx.createBuffer(1, samples.length, tts.sampleRate); buffer.copyToChannel(samples, 0);
    const now = ctx.currentTime; if (!nextAudioTime || nextAudioTime < now + 0.02) nextAudioTime = now + 0.03;
    const source = ctx.createBufferSource(); source.buffer = buffer; source.connect(ctx.destination); source.start(nextAudioTime);
    nextAudioTime += buffer.duration; scheduledAudio.push(source); source.onended = () => { scheduledAudio = scheduledAudio.filter((n) => n !== source); };
  } catch (e) { log(`AUDIO SCHEDULE ERROR: ${e.message}`); }
}

async function loadModel() {
  const button = $("loadModel"); if (!button) return; button.disabled = true; button.textContent = "Loading…"; setProgress(1); const errors = [];
  for (let i = 0; i < MODEL_SOURCES.length; i++) { const base = MODEL_SOURCES[i]; try {
    const bundle = `${base}/english_2026-04`; status(`Loading Pocket TTS mirror ${i + 1}/${MODEL_SOURCES.length}…`, "working"); log(`Trying model mirror: ${bundle}`);
    ...
    tts = candidate; $("modelInfo").textContent = `Ready • ${tts.sampleRate} Hz • INT8 • ${(info?.predefinedVoices || []).length} voices`; $("modelBadge").textContent = "AI ready"; $("modelBadge").style.color = "#83e1aa";
    $("cloneFile").disabled = false; $("recordVoice").disabled = false; setProgress(100); status("Pocket TTS ready. Clone a voice to enable live conversion.", "ok"); button.disabled = false; button.textContent = "Reload Pocket TTS"; log(`Pocket TTS loaded from mirror ${i + 1}.`); await refreshOutputs(); return;
...
async function cloneAudio(blob, name) {
  if (!tts) throw new Error("Load Pocket TTS first."); const ctx = new AudioContext();
  try { const buffer = await ctx.decodeAudioData(await blob.arrayBuffer()); if (buffer.duration < 2) throw new Error("Use at least 2 seconds of speech."); if (buffer.duration > 30) throw new Error("Keep the sample under 30 seconds.");
    const mono = new Float32Array(buffer.length); for (let i = 0; i < buffer.length; i++) { let total = 0; for (let c = 0; c < buffer.numberOfChannels; c++) total += buffer.getChannelData(c)[i] || 0; mono[i] = total / buffer.numberOfChannels; }
...
async function recordClone() {
  ...
  try { const input = await navigator.mediaDevices.getUserMedia({ audio: true }); const recorder = new MediaRecorder(input); const chunks = [];
...
async function speak(text, mode = "manual") {
  if (!tts) { status("Load Pocket TTS first.", "bad"); return; } if (!voiceRef) { status("Clone a voice first.", "bad"); return; }
  const clean = text.trim().slice(0, mode === "conversion" ? 500 : 1500); if (!clean) return;
  if (mode === "conversion") { const generation = conversionGeneration; const ctx = await ensureOutputAudioContext(); status("Converting your voice live…", "working");
    try { await tts.generate(clean, { voice: voiceRef, onChunk: (audio) => enqueueAudioChunk(audio, generation, ctx) }); }
    catch (e) { if (generation === conversionGeneration) { status(`Voice conversion failed: ${e.message}`, "bad"); log(`CONVERSION ERROR: ${e.stack || e.message}`); } } return;
  }
  if (busy) return; busy = true;
  try { status("Generating cloned speech…", "working"); const chunks = []; const metrics = await tts.generate(clean, { voice: voiceRef, onChunk: (audio) => chunks.push(audio.slice()) });
    if (!chunks.length) throw new Error("No audio was generated."); const blob = chunksToWavBlob(chunks, tts.sampleRate); const url = URL.createObjectURL(blob);
    $("preview").src = url; $("preview").hidden = false; $("download").href = url; $("download").download = `voicechanger-${Date.now()}.wav`; $("download").classList.remove("hidden"); await playWav(blob); status(`Spoken • ${(metrics?.audioDuration || 0).toFixed(2)}s`, "ok");
...
function queueCompletedPhrases() { const sentenceMatch = conversionBuffer.match(/^(.+?[.!?…]+)(?:\s+|$)/); if (!sentenceMatch) return false; const phrase = sentenceMatch[1].trim(); conversionBuffer = conversionBuffer.slice(sentenceMatch[0].length).trim(); if (phrase) conversionQueue.push(phrase); return true; }
function flushConversionBuffer(reason = "pause") { if (!conversionBuffer.trim()) return; const phrase = conversionBuffer.trim(); conversionBuffer = ""; conversionQueue.push(phrase); log(`TTS phrase queued (${reason}).`); void drainConversionQueue(); }
function armConversionPauseTimer() { if (conversionTimer) clearTimeout(conversionTimer); if (!conversionBuffer.trim()) { conversionTimer = null; return; } conversionTimer = setTimeout(() => { conversionTimer = null; if (!running || !conversionBuffer.trim()) return; flushConversionBuffer("short pause"); }, 450); }
function queueConversionText(text, final = false) { if (!running || !text) return; const delta = getInterimDelta(lastInterimText, text); lastInterimText = text; if (delta) conversionBuffer = `${conversionBuffer} ${delta}`.trim(); while (queueCompletedPhrases()) {} if (conversionQueue.length) void drainConversionQueue(); if (final) { lastInterimText = ""; if (conversionTimer) { clearTimeout(conversionTimer); conversionTimer = null; } flushConversionBuffer("Deepgram final"); return; } armConversionPauseTimer(); }
...
async function startVC() {
  if (!apiKey()) throw new Error("Enter your Deepgram API key first."); if (!voiceRef) throw new Error("Clone a voice first.");
  manualStop = false; reconnectGeneration++; reconnectAttempts = 0;
