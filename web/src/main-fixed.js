...
// These are BASE onnx/ directories. pocket-tts-js adds /english_2026-04.
// The first two are independent mirrors so a Hugging Face 429 does not stop loading.
const MODEL_SOURCES = [
...
function voiceStatus(message, type = "") {
  const el = $("voiceStatus");
  if (el) { el.textContent = message; el.className = `status ${type}`; }
}
function apiKey() {
  return $("deepgramKey")?.value.trim() || localStorage.getItem("voicechanger.deepgramKey") || "";
}
...
async function refreshOutputs() {
  ...
  try {
    ...
    for (const device of outputs) {
      if (device.deviceId !== "default") select.add(new Option(device.label || "Audio output", device.deviceId));
    }
...
function stopPlayback() {
  if (playback) {
    try { playback.pause(); playback.removeAttribute("src"); playback.load(); } catch {}
    playback = null;
  }
...
async function loadModel() {
  ...
  for (let i = 0; i < MODEL_SOURCES.length; i++) {
    ...
    try {
      ...
      const candidate = new PocketTTS({
        ...
        voiceCloning: true,
        cache: true,
        cacheName: "voicechanger-pocket-tts-v6",
  ...
  status("Pocket TTS could not load from any mirror.", "bad");
...
async function cloneAudio(blob, name) {
  ...
  try {
    ...
    for (let i = 0; i < buffer.length; i++) {
      let total = 0;
      for (let c = 0; c < buffer.numberOfChannels; c++) total += buffer.getChannelData(c)[i] || 0;
      mono[i] = total / buffer.numberOfChannels;
    }
...
async function cloneFile(file) {
  ...
  catch (e) { voiceStatus(`Clone failed: ${e.message}`, "bad"); log(`CLONE ERROR: ${e.message}`); }
...
async function recordClone() {
  ...
  try {
    ...
    recorder.onstop = async () => {
      ...
      catch (e) { voiceStatus(`Clone failed: ${e.message}`, "bad"); log(`MIC CLONE ERROR: ${e.message}`); }
    };
    recorder.start();
...
async function speak(text, mode = "manual") {
  ...
  try {
    ...
    await playWav(blob);
    status(`Spoken • ${(metrics?.audioDuration || 0).toFixed(2)}s`, "ok");
  } catch (e) {
...
function stopVC() {
  ...
  $("startVC").disabled = !voiceRef || !apiKey();
...
async function startVC() {
  ...
  socket = new WebSocket(
    "wss://api.deepgram.com/v1/listen?model=nova-3&language=en-US&encoding=linear16&sample_rate=16000&channels=1&interim_results=true&smart_format=true&endpointing=300",
    ["token", apiKey()]
  );
  ...
  socket.onopen = () => {
    ...
    processor.onaudioprocess = (event) => {
      ...
      socket.send(pcm16(downsample(event.inputBuffer.getChannelData(0), audioContext.sampleRate)).buffer);
    };
    running = true;
  ...
  socket.onmessage = (event) => {
    ...
    if (data.is_final) echoQueue = echoQueue.then(() => speak(text, "echo")).catch((e) => log(`ECHO ERROR: ${e.message}`));
  };
  socket.onerror = () => log("Deepgram WebSocket error.");
  socket.onclose = () => { if (running) stopVC(); };
}
...
$("deepgramKey").addEventListener("input", () => {
  ...
  if (voiceRef) $("startVC").disabled = !apiKey();
});
...
$("stop").addEventListener("click", () => { try { tts?.stop?.(); } catch {} stopPlayback(); });
