...
class Handler(BaseHTTPRequestHandler):
 ...
 def _send(self,code,payload):
  data=payload if isinstance(payload,bytes) else json.dumps(payload).encode(); self.send_response(code); self.send_header('Content-Type','application/json'); self.send_header('Access-Control-Allow-Origin','*'); self.send_header('Access-Control-Allow-Headers','content-type'); self.send_header('Access-Control-Allow-Methods','GET,POST,OPTIONS'); self.send_header('Content-Length',str(len(data))); self.end_headers(); self.wfile.write(data)
 def do_OPTIONS(self): self._send(204,b'')
 def do_GET(self): self._send(200,{'ok':True,'engine':'OpenVoice V2'})
 def do_POST(self):
  if self.path!='/generate': return self._send(404,{'error':'Not found'})
  try:
   n=int(self.headers.get('Content-Length','0'))
   if n>40_000_000: raise ValueError('Request is too large.')
   body=json.loads(self.rfile.read(n)); text=str(body.get('text','')).strip(); ref=body.get('reference_base64','')
   ...
   with tempfile.TemporaryDirectory(prefix='vc-echo-') as td:
    ...
    if getattr(sys,'frozen',False): cmd=[str(Path(sys.executable).with_name('openvoice-engine.exe')),'--ref-audio',str(refpath),'--text',text,'--output',str(out),'--language','English']
    else: cmd=[os.environ.get('PYTHON','python'),str(ROOT.parent/'openvoice'/'engine.py'),'--ref-audio',str(refpath),'--text',text,'--output',str(out),'--language','English']
