...
function addDevice(select,d,labelExtra=""){const o=document.createElement("option");o.value=d.id;o.textContent=(d.name||"Audio device")+labelExtra;select.appendChild(o)}
...
start.onclick=async()=>{if(running)return;try{const h=await health();if(!h?.voice){say("Load a reference WAV first.");return}if(!mic.value){say("Choose a microphone.");return}if(!out.value){say("Choose an output device.");return}await connect();ws.send(JSON.stringify({type:"start",inputId:mic.value,outputId:out.value}));running=true;start.disabled=true;stop.disabled=false;say(String(out.options[out.selectedIndex]?.textContent||"Output")+" selected. Starting...")}catch(e){say("Start failed: "+e.message);try{ws?.close()}catch{}}};
