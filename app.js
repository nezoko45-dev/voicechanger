const $=id=>document.getElementById(id),mic=$("mic"),out=$("out"),voice=$("voice"),load=$("load"),start=$("start"),stop=$("stop"),status=$("status"),msg=$("msg");
...
async function devices(){
  try{const j=await fetch("/devices").then(r=>r.json());if(!j.ok)throw Error(j.error);const keepMic=mic.value,keepOut=out.value;mic.innerHTML="";out.innerHTML="";
    for(const d of j.devices){let label=d.name||"Audio device";if(d.inputChannels>0)label+=" [input "+d.inputChannels+"]";if(d.outputChannels>0)label+=" [output "+d.outputChannels+"]";
      if(d.inputChannels>0){const o=document.createElement("option");o.value=d.id;o.textContent=label;mic.appendChild(o)}
      if(d.outputChannels>0){const o=document.createElement("option");o.value=d.id;o.textContent=label;out.appendChild(o)}
    }
    if([...mic.options].some(x=>x.value===keepMic))mic.value=keepMic;if([...out.options].some(x=>x.value===keepOut))out.value=keepOut;
