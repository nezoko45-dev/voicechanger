...
let server, nativePlayer = null, nativeTemp = null, nextDeepgramId = 1;
const deepgramSockets = new Map();

function stopNativePlayer(){ if(nativePlayer){try{nativePlayer.kill();}catch{} nativePlayer=null;} if(nativeTemp){try{fs.unlinkSync(nativeTemp);}catch{} nativeTemp=null;} }
function playNativeWav(base64){
  ...
  stopNativePlayer(); const file=path.join(os.tmpdir(),`voicechanger-${process.pid}-${Date.now()}.wav`); fs.writeFileSync(file,Buffer.from(base64,'base64')); nativeTemp=file;
  const escaped=file.replace(/'/g,"''"); const script=`$p = New-Object System.Media.SoundPlayer('${escaped}'); $p.PlaySync()`;
  ...
  nativePlayer.on('exit',()=>{nativePlayer=null;if(nativeTemp===file){try{fs.unlinkSync(file);}catch{}nativeTemp=null;}}); return true;
}
...
function runPowerShellScript(action,elevated=false){
  ...
  return new Promise((resolve,reject)=>{const command=elevated?`$p = Start-Process -FilePath 'powershell.exe' -Verb RunAs -Wait -PassThru -ArgumentList '-NoProfile','-ExecutionPolicy','Bypass','-File','${escapedScript}','${action}'; exit $p.ExitCode`:`& powershell.exe -NoProfile -ExecutionPolicy Bypass -File '${escapedScript}' '${action}'; exit $LASTEXITCODE`;const child=spawn('powershell.exe',['-NoProfile','-NonInteractive','-ExecutionPolicy','Bypass','-Command',command],{windowsHide:true,stdio:['ignore','pipe','pipe']});let stdout='',stderr='';child.stdout.on('data',d=>stdout+=d.toString());child.stderr.on('data',d=>stderr+=d.toString());child.on('error',reject);child.on('exit',code=>{const text=`${stdout}\n${stderr}`.trim(),status=classifyDriverOutput(text);if(code===0||status.reboot||status.installed||status.staged)resolve({ok:true,output:text,code:code??0,...status});else resolve({ok:false,output:text||`PowerShell exited with code ${code}`,code,...status});});});
}
...
app.on('uncaughtException',e=>console.error('[VoiceChanger main process]',e));process.on('unhandledRejection',r=>console.error('[VoiceChanger unhandled rejection]',r));
app.whenReady().then(async()=>{session.defaultSession.setPermissionRequestHandler((_wc,permission,callback)=>callback(['media','microphone','speaker-selection','notifications'].includes(permission)));try{await createWindow();}catch(e){console.error('[VoiceChanger] window startup failed:',e);}app.on('activate',()=>{if(BrowserWindow.getAllWindows().length===0)createWindow().catch(console.error);});});
