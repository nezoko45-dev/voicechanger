...
# WAV -> Faster-Whisper transcription -> XTTS-v2 synthesis using the WAV as
# the speaker reference. This is real neural speech synthesis, not pitch DSP.
...
COMPUTE_TYPE = os.getenv("PYTHON_TTS_COMPUTE_TYPE", "float16" if DEVICE.startswith("cuda") else "int8")
...
@app.get("/health")
def health():
    return jsonify({"ok": True, "device": DEVICE, "whisper": WHISPER_SIZE, "tts": "xtts_v2"})
...
def synthesize():
    ...
    if uploaded.content_length and uploaded.content_length > 50 * 1024 * 1024:
    ...
    with tempfile.TemporaryDirectory(prefix="uwu_tts_") as directory:
        ...
        try:
            ...
            # XTTS uses the uploaded WAV as the speaker reference. The spoken
            # words are regenerated by the TTS model instead of replaying the
            # source audio.
            tts.tts_to_file(
                text=text,
            ...
            response = send_file(str(output), mimetype="audio/wav", as_attachment=False)
